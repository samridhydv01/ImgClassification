...
def upload_file():
    ...
    if request.method == 'POST':
        ...
        pred_strings = []
        for pred_class, pred_prob in zip(predictions[0], predictions[1]):
            pred_strings.append(str(pred_class).strip() + " : " + str(torch.round(torch.tensor(pred_prob) * 10**5) / 10**5).strip())
